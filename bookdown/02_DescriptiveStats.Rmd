...
Based on the final row of data, there are `r max(totals$cumulative)` unique cases of herbicide resistance in this data set. And now, we can use the new data frame generated by `dplyr` to make a plot of cumulative herbicide resistance cases over time, coded by herbicide mode of action. The specifics of plotting are discussed in the Data Visualization section. 

(ref:figcanadahrcap) _Cumulative number of herbicide resistant weed cases in Canada over time._
```{r figcanadahr, fig.asp=1, fig.cap='(ref:figcanadahrcap)'}
